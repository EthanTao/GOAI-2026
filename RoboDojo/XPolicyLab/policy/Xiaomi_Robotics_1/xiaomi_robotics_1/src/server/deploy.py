...
def unwrap_checkpoint(state_dict):
    if not isinstance(state_dict, dict):
        raise TypeError(f"checkpoint is {type(state_dict).__name__}, expected dict")
    for wrapper in ("module", "state_dict", "model_state"):
        inner = state_dict.get(wrapper)
        if isinstance(inner, dict) and inner:
            print(f"[deploy] unwrapping checkpoint key {wrapper!r} ({len(inner)} entries)")
            return unwrap_checkpoint(inner)
    return state_dict
...
def build_model(cfg, model_path):
    ...
    allowed = {
        name
        for name, param in inspect.signature(XR1.__init__).parameters.items()
        if name != "self" and param.kind != inspect.Parameter.VAR_KEYWORD
    }
    dropped = sorted(set(model_cfg) - allowed)
    if dropped:
        print(f"[deploy] ignoring config keys not in XR1.__init__: {dropped}")
        model_cfg = {k: v for k, v in model_cfg.items() if k in allowed}
    model = XR1(**model_cfg).to(torch.bfloat16)
    print(f"[deploy] loading checkpoint {model_path}")
    state_dict = torch.load(model_path, map_location="cpu", weights_only=False)
    if isinstance(state_dict, dict):
        print(f"[deploy] checkpoint top-level keys: {list(state_dict.keys())[:16]}")
    state_dict = unwrap_checkpoint(state_dict)
    prefixed = format_state_dict(state_dict, "model.")
    if prefixed:
        print(f"[deploy] stripped 'model.' prefix: {len(prefixed)}/{len(state_dict)} tensors")
        state_dict = prefixed
    else:
        sample = list(state_dict.keys())[:8]
        print(f"[deploy] no 'model.' prefix; loading {len(state_dict)} keys as-is, sample={sample}")
    model.eval()
